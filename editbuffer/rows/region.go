...
func (rs *Rows) RemoveRegion(start, end screen.Cursor) Rows {
...
// The region is half-open: [start, end).
// No undo/redo functionality is provided by this package.
func (rs *Rows) GetRegion(start, end screen.Cursor) Rows {
	return rs.region(start, end, false)
}

/*
// region returns the bytes in the specified region.
// When doRemove is true, the region is also removed from the rows.
//
// colIndex is a byte offset within a row.
// len(row) is a valid row-end position.
// func (rs *Rows) region(start, end screen.Cursor, doRemove bool) []byte {
func (rs *Rows) region(start, end screen.Cursor, doRemove bool) rows.Rows {
	...
	if start.RowIndex == end.RowIndex {
		removed := topRow.CopyBytes(start.ColIndex, end.ColIndex)
	...
	//   remaining bytes in the top row
	//   + all bytes in the middle rows
	//   + bytes removed from the bottom row
	capacity := topRow.Length() - start.ColIndex

	for i := start.RowIndex + 1; i < end.RowIndex; i++ {
		capacity += rs.Row(i).Length()
	}

	capacity += end.ColIndex

	removed := make([]byte, 0, capacity)

	// Top row: remove everything from start.ColIndex to the end.
	removed = append(removed, topRow.Bytes()[start.ColIndex:]...)
	...
	// Middle rows.
	for i := start.RowIndex + 1; i < end.RowIndex; i++ {
		removed = append(removed, rs.Row(i).Bytes()...)
	}

	// Bottom row: remove everything from the beginning to end.ColIndex.
	removed = append(removed, bottomRow.Bytes()[:end.ColIndex]...)
...
// colIndex is a byte offset within a row.
// len(row) is a valid row-end position.
func (rs *Rows) region(start, end screen.Cursor, doRemove bool) Rows {
	// Validate row indices.
	if start.RowIndex < 0 ||
		end.RowIndex < 0 ||
		start.RowIndex > end.RowIndex ||
